...
func (w *worker) commitTransaction(ctx context.Context, vmenv *vm.EVM, tx *types.Transaction, signer types.Signer) ([]*types.Log, error) {
	snap := w.current.state.Snapshot()

	receipt, _, err := core.ApplyTransaction(ctx, vmenv, w.config, w.current.gasPool, w.current.state, w.current.header, tx, &w.current.header.GasUsed, signer)
	if err != nil {
...
func (w *worker) commitTransactions(ctx context.Context, txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	for {
		...
		logs, err := w.commitTransaction(ctx, vmenv, tx, signer)
		switch err {
