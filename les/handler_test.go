...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	case 2:
		...
		trie, _ := trie.New(root, trie.NewDatabase(ethdb.NewTable(server.db.GlobalTable(), light.ChtTablePrefix)))
...
func TestGetBloombitsProofs(t *testing.T) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(root, trie.NewDatabase(common.NewTablePrefixer(server.db.GlobalTable(), light.BloomTrieTablePrefix)))
