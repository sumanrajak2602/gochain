...
func (t *rlpx) close(err error) {
	...
	if t.rw != nil {
		if r, ok := err.(DiscReason); ok && r != DiscNetworkError {
			...
			} else if err := SendItems(t.rw, discMsg, r); err != nil {
...
// and also verifies whether the encryption handshake 'worked' and the
// remote side actually provided the right public key.
func (t *rlpx) doEncHandshake(ctx context.Context, prv *ecdsa.PrivateKey, dial *ecdsa.PublicKey) (*ecdsa.PublicKey, error) {
	...
	if dial == nil {
		sec, err = receiverEncHandshake(ctx, t.fd, prv)
	} else {
...
//
// prv is the local client's private key.
func receiverEncHandshake(ctx context.Context, conn io.ReadWriter, prv *ecdsa.PrivateKey) (s secrets, err error) {
