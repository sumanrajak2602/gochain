...
func SetupGenesisBlock(db common.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	...
	if storedcfg == nil {
		...
		rawdb.WriteChainConfig(db.GlobalTable(), stored, newcfg)
	...
	height := rawdb.ReadHeaderNumber(db.GlobalTable(), rawdb.ReadHeadHeaderHash(db.GlobalTable()))
	if height == nil {
...
func (g *Genesis) Commit(db common.Database) (*types.Block, error) {
	...
	if err := rawdb.WriteTd(db.GlobalTable(), block.Hash(), block.NumberU64(), g.Difficulty); err != nil {
		return nil, err
	}
	if err := rawdb.WriteBlock(db.GlobalTable(), db.BodyTable(), db.HeaderTable(), block); err != nil {
		return nil, err
	}
	if err := rawdb.WriteBlockReceipts(db.ReceiptTable(), block.Hash(), block.NumberU64(), nil); err != nil {
		return nil, err
	}
	if err := rawdb.WriteCanonicalHash(db, block.Hash(), block.NumberU64()); err != nil {
		return nil, err
	}
	if err := rawdb.WriteHeadBlockHash(db.GlobalTable(), block.Hash()); err != nil {
		return nil, err
	}
	if err := rawdb.WriteHeadHeaderHash(db.GlobalTable(), block.Hash()); err != nil {
		return nil, err
	}
	config := g.Config
	if config == nil {
	...
	return block, rawdb.WriteChainConfig(db.GlobalTable(), block.Hash(), config)
}
