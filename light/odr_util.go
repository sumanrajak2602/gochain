...
func GetHeaderByNumber(ctx context.Context, odr OdrBackend, number uint64) (*types.Header, error) {
	...
	if (hash != common.Hash{}) {
		...
		header := rawdb.ReadHeader(db.HeaderTable(), hash, number)
		if header == nil {
...
func GetBodyRLP(ctx context.Context, odr OdrBackend, hash common.Hash, number uint64) (rlp.RawValue, error) {
	if data := rawdb.ReadBodyRLP(odr.Database().BodyTable(), hash, number); data != nil {
		return data, nil
	}
...
func GetBlock(ctx context.Context, odr OdrBackend, hash common.Hash, number uint64) (*types.Block, error) {
	...
	header := rawdb.ReadHeader(odr.Database().HeaderTable(), hash, number)
	if header == nil {
...
func GetBlockReceipts(ctx context.Context, odr OdrBackend, hash common.Hash, number uint64) (types.Receipts, error) {
	...
	receipts := rawdb.ReadReceipts(odr.Database().ReceiptTable(), hash, number)
	if receipts == nil {
	...
	if len(receipts) > 0 && receipts[0].TxHash == (common.Hash{}) {
		...
		config := rawdb.ReadChainConfig(odr.Database().GlobalTable(), genesis)

		if err := core.SetReceiptsData(ctx, config, block, receipts); err != nil {
			return nil, err
		}
		rawdb.WriteReceipts(odr.Database().ReceiptTable(), hash, number, receipts)
	}
	return receipts, nil
}
