...
type ChtIndexerBackend struct {
	diskdb, trieTable    common.Database
	odr                  OdrBackend
...
func NewChtIndexer(db common.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := common.NewTablePrefixer(db.GlobalTable(), ChtTablePrefix)
	...
	return core.NewChainIndexer(db, common.NewTablePrefixer(db.GlobalTable(), "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func (c *ChtIndexerBackend) Process(ctx context.Context, header *types.Header) error {
	...
	td := rawdb.ReadTd(c.diskdb.GlobalTable(), hash, num)
	if td == nil {
		panic(nil)
	}
	var encNumber [8]byte
...
func NewBloomTrieIndexer(db common.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := common.NewTablePrefixer(db.GlobalTable(), BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, common.NewTablePrefixer(db.GlobalTable(), "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}
