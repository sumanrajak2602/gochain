...
func (bc *LightChain) ResetWithGenesisBlock(genesis *types.Block) {
	...
	rawdb.WriteBlock(bc.chainDb.GlobalTable(), bc.chainDb.BodyTable(), bc.chainDb.HeaderTable(), genesis)
...
func (self *LightChain) InsertHeaderChain(ctx context.Context, chain []*types.Header, checkFreq int) (int, error) {
	start := time.Now()
	if i, err := self.hc.ValidateHeaderChain(ctx, chain, checkFreq); err != nil {
		return i, err
	}
...
//TODO SubscribeLogsEvent

//TODO SubscribeRemovedLogsEvent
